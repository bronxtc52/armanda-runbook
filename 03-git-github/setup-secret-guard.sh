...
cat > "$HOOK" <<'HOOK_EOF'
#!/usr/bin/env bash
# Глобальный pre-commit: не пускать секреты в коммит.
# Поставлен course-setup-runbook (фаза 03). Блокирует коммит ТОЛЬКО когда
# gitleaks уверенно нашёл утечку (код 1); при ошибке инструмента — пропускает
# с предупреждением, чтобы не мешать работать.
