...
# _runner_note "текст" — информ-веха для приложения (НЕ стоп-точка). Маркер
# @@RUNNER:NOTE:<текст>@@ приложение показывает чекпойнтом в ленте, НЕ меняя
# статус раннера (в отличие от NEED_HUMAN — не приглашает жать «продолжай», нет
# риска лишнего Enter в PTY-буфер). Нужно, чтобы события вроде «сейчас появится
# нативное окно пароля» не были тихими: если окно macOS всплыло за приложением
# или на другом рабочем столе, пользователь всё равно увидит подсказку в чате.
_runner_note() {
  [ "${RUNNER_PTY:-}" = "1" ] || return 0
  printf '@@RUNNER:NOTE:%s@@\n' "$*"
}
...
# Под раннером пароль спрашивает НАТИВНОЕ окно macOS (через SUDO_ASKPASS +
# run_sudo), а не поле в приложении — поэтому здесь НЕ эмитим NEED_SECRET.
# В Терминале (без раннера) пароль вводится там же, где идёт sudo.
warn_password() {
  if [ -n "${SUDO_ASKPASS:-}" ]; then
    # Информ-веха в приложение: если нативное окно macOS всплывёт за окном
    # приложения или на другом Space, пользователь всё равно увидит подсказку —
    # раньше зависший sudo был тихим (диалог не виден → всё «встало»).
    _runner_note "Сейчас появится системное окно с запросом пароля от Mac. Если его не видно — поищи его за окном приложения или на другом рабочем столе."
    warn "Сейчас появится окно macOS с запросом пароля от компьютера."
    note "Это стандартное системное окно (как при установке программ). Введи пароль и нажми OK."
  else
...
# run_sudo <команда…> — запустить команду с правами администратора.
# Под раннером (SUDO_ASKPASS задан) форсируем нативное окно пароля macOS
# флагом -A; в Терминале у человека — обычный sudo с вводом в tty. Так один и
# тот же runbook работает и в приложении, и при прямом запуске курса.
run_sudo() {
  if [ -n "${SUDO_ASKPASS:-}" ]; then
    sudo -A "$@"
  else
